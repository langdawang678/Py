...
# 数据表-数据行-字段 对应 类-对象-属性


# 以下from https://www.cnblogs.com/jiangmingbai/p/10909449.html  结合了 c20b01:00:00的注释等.做了修改
# 创建父类用于,统一字段的类型
# 用于元类创建类的时候判断属性类型
class BaseField:
    pass
...
class FieldMateClass(type):
    ...
    def __new__(cls, name, bases, attrs, *args, **kwargs):
        ...
        if name == "BaseModel":
            ...
            table_name = name.lower()  # 类名对应数据表(表名),通常为转成小写
            # 生成字段和表的映射关系-属性都保存在attrs中
            fields = {}  # 存放模型类字段和数据表中字段对应的关系
